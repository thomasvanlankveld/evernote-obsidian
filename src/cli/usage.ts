...
export function usage(): string {
  return [
    ...
    '  evernote-obsidian correlate --snapshot <path> [--vault-dir <path>] [--overrides <path>] [--out <path>] [--map-out <path>] [--report <path>] [--no-report-md] [--verbose]',
    ...
    '  --snapshot                     Path to Evernote snapshot JSON (required for correlate; check defaults to ./out/evernote-notes.json when omitted)',
    '  --json                         Machine-readable JSON on stdout (check; non-TTY stdout also uses JSON)',
    ...
    '  With run, provide --db or --snapshot for Evernote metadata (or --map alone to rerun repair only). --map without --snapshot skips snapshot and correlate; --db is ignored in that case.',
    ...
    '  --no-report-md                 On correlate failure, skip writing the human-readable .md report beside --report',
    ...
    '',
    '  On correlate failure (exit 1), stderr shows a hint, vault/Evernote context, and next steps; TTY runs also point at correlate-report.md. Compact summary JSON (includes reportMarkdownPath when set) is written when stderr is not a TTY; use --verbose for full report JSON on stderr.',
