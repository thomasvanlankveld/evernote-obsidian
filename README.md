...
Copy `.env.example` to `.env` and set **`EVERNOTE_CONSUMER_KEY`** / **`EVERNOTE_CONSUMER_SECRET`**. Run **`login`** once (per account / expiry) so **`snapshot`** can read the saved OAuth token file. Optional **`EVERNOTE_HOST`** selects production (`www.evernote.com`), sandbox, or Yinxiang (`app.yinxiang.com`). Evernote’s classic API uses **OAuth 1** long-lived access tokens (with `edam_expires`); there is **no refresh token** — run **`login`** again before expiry if `snapshot` reports the token expired.
...
- **Authentication:** **OAuth 1** only — **`evernote-obsidian login`** (consumer key + secret from Evernote’s developer site) writes a gitignored token file under **`out/`** by default; **`snapshot`** reads it.
...
- **Evernote policy:** API access follows Evernote’s developer and product rules; if the classic API or OAuth flow changes, this tool may need updates.
...
- Never commit OAuth access token files, consumer secrets, or `.enex` files that contain private notes unless this repo is strictly private and you accept the risk.
